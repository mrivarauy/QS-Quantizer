...
QUANT="$1$2"
PREFIX=${QUANT}-${FRACTION}-1 # files prefix (change for original or 4bin run, and change last number for replicates)
NPROC=35 #number of threads to use
READS_ORI=./input/HG00733_2-${FRACTION}.fastq #non quantized fastq file
READS_QUAN=./input/HG00733_2-${QUANT}-${FRACTION}.fastq #quantized fastq file 
...
if [ $wtdbgflag == true ]; then
	...
	cd wtdbg2-${FRACTION}
	wtdbg2 -t ${NPROC} -x ont -L 10000 -g 3.3g -i ../${READS_ORI} -o wtdbg2-assembly-${FRACTION}
	source activate human-env
...
if [ $marginflag == true ]; then
	...
	minimap2 -ax map-ont -t ${NPROC} ./wtdbg2-${FRACTION}/${ASSEMBLY} ${READS_QUAN} | samtools sort -@ ${NPROC} | samtools view -hb -F 0x104 > ./wtdbg2-${FRACTION}/${QUANT}-${ASSEMBLY}.bam #mapping reads to assembly with minimap2
	samtools index -@ ${NPROC} ./wtdbg2-${FRACTION}/${QUANT}-${ASSEMBLY}.bam
	${MP_PROGRAM}/marginPolish ./wtdbg2-${FRACTION}/${QUANT}-${ASSEMBLY}.bam ./wtdbg2-${FRACTION}/${ASSEMBLY} ${MP_PARAMS} -t ${NPROC} -o ./wtdbg2-${FRACTION}/MP-${PREFIX} -f
