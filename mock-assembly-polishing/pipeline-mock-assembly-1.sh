...
#Set up input data
BASE="./bins/${PREFIX}" 
REF_DIR_ILL="./ref-dir" #assembly reference, used for performance evaluation
...
MEDAKA_MODEL="/media/Disco9/mrivara/cuantizacion-nanoporos/pruebas-github/prueba-final/mock/medaka-model/r103_min_high_g345_model.hdf5" #used medaka model
...
if [ $flyeflag == true ]; then
	...
	seqtk seq ${OUT_DIR}/assembly-${PREFIX}/assembly.fasta | 
...
if [ $raconflag == true ]; then
    ...
    ../racon-v1.3.2/build/bin/racon -t ${NPROC} ${READS_QUAN} ${OUT_DIR}/map1-${PREFIX}.sam ${ASSEMBLY} > ${OUT_DIR}/racon_r1-${PREFIX}.fasta
    minimap2 -ax map-ont -t ${NPROC} ${OUT_DIR}/racon_r1-${PREFIX}.fasta ${READS_QUAN} > ${OUT_DIR}/map2-${PREFIX}.sam
    ../racon-v1.3.2/build/bin/racon -t ${NPROC} ${READS_QUAN} ${OUT_DIR}/map2-${PREFIX}.sam ${OUT_DIR}/racon_r1-${PREFIX}.fasta > ${OUT_DIR}/racon_r2-${PREFIX}.fasta
fi
...
if [ $marginflag == true ]; then
	if [ -f ${OUT_DIR}/map1-${PREFIX}.sam ]; then
	    samtools view -T ${ASSEMBLY} -F 2308 -Sb ${OUT_DIR}/map1-${PREFIX}.sam | samtools sort -@ ${NPROC} -o ${ASSEMBLY}.bam
	else
	...
	../marginPolish/build/marginPolish ${ASSEMBLY}.bam ${ASSEMBLY} ${MP_PARAMS} -t ${NPROC} -o ${OUT_DIR}/MP-${PREFIX} -f
